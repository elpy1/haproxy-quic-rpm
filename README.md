...
| --- | --- | --- |
| haproxy-quic | el9 | [AWS-LC](https://github.com/aws/aws-lc) 5.8.0 |
...
```bash
make rpm-build HAPROXY_VERSION=3.2.23 AWS_LC_VERSION=5.8.0
```
...
```bash
make rpm-build HAPROXY_VERSION=3.2.23 AWS_LC_VERSION=5.8.0 PACKAGE_RELEASE=1
```
...
```bash
make rpm-build AWS_LC_SSL_RUNNER_WORKERS=1
```
...
RPMS
└── x86_64
    ├── haproxy-quic-3.2.23-1.aws_lc.5.8.0.el9.x86_64.rpm
    ├── haproxy-quic-debuginfo-3.2.23-1.aws_lc.5.8.0.el9.x86_64.rpm
    └── haproxy-quic-debugsource-3.2.23-1.aws_lc.5.8.0.el9.x86_64.rpm
SRPMS
└── haproxy-quic-3.2.23-1.aws_lc.5.8.0.el9.src.rpm
```
...
- the matching debuginfo and debugsource RPMs
- the matching SRPM
- a `SHA256SUMS` file for the attached assets

Tags should use the form `v<HAPROXY_VERSION>-aws-lc-<AWS_LC_VERSION>`, for example `v3.2.23-aws-lc-5.8.0`.
...
```
dnf install /path/to/haproxy-quic-3.2.23-1.aws_lc.5.8.0.el9.x86_64.rpm
```
...
AWS-LC versions are recorded exactly as upstream tags them, without the leading `v`.
Starting with AWS-LC `5.0.0`, upstream documents that AWS-LC does not use Semantic Versioning: mainline releases use major/minor increments, and a mainline major bump means an LTS branch was cut rather than necessarily indicating an API or ABI break.
Patch versions are reserved for FIPS branches.
...
- `haproxy-quic-3.2.22-1.aws_lc.5.5.0.el9`
- `haproxy-quic-3.2.23-1.aws_lc.5.8.0.el9`

Reset `PACKAGE_RELEASE` to `1` when `HAPROXY_VERSION` changes. While `HAPROXY_VERSION` stays the same, the leading `PACKAGE_RELEASE` value must keep increasing.
