...
| --- | --- | --- |
| haproxy-quic | el9 | [AWS-LC](https://github.com/aws/aws-lc) 5.5.0 |
...
```bash
make rpm-build HAPROXY_VERSION=3.2.22 AWS_LC_VERSION=5.5.0
```
...
```bash
make rpm-build HAPROXY_VERSION=3.2.22 AWS_LC_VERSION=5.5.0 PACKAGE_RELEASE=1
```
...
RPMS
└── x86_64
    ├── haproxy-quic-3.2.22-1.aws_lc.5.5.0.el9.x86_64.rpm
    ├── haproxy-quic-debuginfo-3.2.22-1.aws_lc.5.5.0.el9.x86_64.rpm
    └── haproxy-quic-debugsource-3.2.22-1.aws_lc.5.5.0.el9.x86_64.rpm
SRPMS
└── haproxy-quic-3.2.22-1.aws_lc.5.5.0.el9.src.rpm
```
...
Tags should use the form `v<HAPROXY_VERSION>-aws-lc-<AWS_LC_VERSION>`, for example `v3.2.22-aws-lc-5.5.0`.
...
```
dnf install /path/to/haproxy-quic-3.2.22-1.aws_lc.5.5.0.el9.x86_64.rpm
```
